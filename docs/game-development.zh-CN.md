...
状态、输入和像素结果。本仓库不依赖 ESP-Iris，也不走 ESP-Mosaico Vibe 的
`mosaico.py game` 或 `tools/gsp-sim`。LCD 时序、触摸手感和音频仍需真机验收。
...
示例是普通 ESP-IDF 工程，不经过 ESP-Iris 或 Recovery。板级支持由组件管理器从
https://github.com/esp-mosaico/esp-mosaico-bsp.git 拉取。GSP 场景编译器可选
`MOSAICO_GAME_GSPC_FETCHER`，或 PATH 上的 `gspc` / `gspc-dev`，版本对齐
espressif/esp-gsp 1.4.0。

16MB flash 上 `factory` 为 5MB（`0x20000`），以便 `idf.py flash` 装下
Living Worlds（约 3.8MB）和 Last Zone（约 2.1MB）。带 `game_assets` 的示例把
该分区留在 `0xf00000`。
