...
void DrawMosaicoTilemapLayer(MosaicoTilemap m,uint32_t layer,Rectangle viewport){if(!m||!m->used||layer>=m->h->layers)return;int x0=(int)viewport.x/m->h->tile_width,y0=(int)viewport.y/m->h->tile_height,x1=(int)(viewport.x+viewport.width+m->h->tile_width-1)/m->h->tile_width,y1=(int)(viewport.y+viewport.height+m->h->tile_height-1)/m->h->tile_height;if(x0<0)x0=0;if(y0<0)y0=0;if(x1>m->h->width)x1=m->h->width;if(y1>m->h->height)y1=m->h->height;const uint16_t*tiles=m->layers+(size_t)layer*m->h->width*m->h->height;for(int y=y0;y<y1;++y)Mosaico2DDrawTileRow(m->atlas.texture,&tiles[(size_t)y*m->h->width+x0],(size_t)(x1-x0),m->h->tile_width,m->h->tile_height,x0*m->h->tile_width,69+y*m->h->tile_height);}
